			...
			connection.query(`SELECT * FROM Users WHERE UserID="${author.id}"`, (err, results) => {
                ...
                if(!results[0]){
                    ...
                    const AttrEmbed = new RichEmbed()
                        ...
                        .addField(`Strength`, `${attribs.Str} [+${(attribs.Str / 75).toFixed(2)} DMG]`)
                        .addField(`Accuracy`, `${attribs.Acc} [+${(attribs.Acc / 1000).toFixed(2)} ACC]`,true)
                        .addField(`Intelligence`, `${attribs.Int} [+${(attribs.Int / 75).toFixed(2)} M.DMG]`,true)
                        .addField(`Vitality`, `${attribs.Vit} [+${(attribs.Vit / 10).toFixed(2)} HP]`,true)
                        .addField(`Resistance`, `${attribs.Res} [+${(attribs.Res / 50).toFixed(2)} RES]`,true)
