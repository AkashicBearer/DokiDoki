    ...
    async run(message, args){
                ...
                if(mes.content.toLowerCase() == word){
                   ...
                   for(var i = 0; i<wowi.length; i++){
                    ...
                    if(mes.content.length == 1){
                        ...
                        message.channel.send("Please only guess one letter at a time, type `cancel` to cancel.")
                        guess(word)
                    }
        ...
        function win(){
            var money = Math.round((Math.random()+1)*10)
                        ...
                        pool.query(`SELECT arcanium FROM xp WHERE userid = '${message.author.id}'`,(err, result) => {
                            let arcanium = result.rows[0].arcanium
                            pool.query(`UPDATE xp SET arcanium = ${arcanium + money} WHERE userid='${message.author.id}'`)
                             pool.end(err => {
                              if(err) throw err; 
                             })
                        })
